...
- ルーレット: ページを開いた時点で盤面が見えるか。初回から回転し、ときどき隣の候補で止まりそうになってから一コマ進むか。通常停止とフェイント付き停止の両方で、表示結果が針の位置と一致するか。OSの「動きを減らす」が有効でも回転し、ページ内のチェックを外すと回転を省略できるか。2件、20件、空行、同じ候補、41文字の入力も試す。
